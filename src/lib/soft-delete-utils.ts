...
export async function getStudentRelatedRecords(studentId: string): Promise<RelatedRecordCount> {
  // Get student with class info for enrollment analysis
  ...
  const autoGeneratedEnrollments = student?.classId
    ? allEnrollments.filter(e => e.classSection?.subject?.classId === student.classId)
    : [];
 ...
 * Only removes enrollments that were automatically created based on the student's class
 */
export async function cleanupAutoGeneratedEnrollments(studentId: string): Promise<{
  ...
  try {
    ...
    // Find auto-generated enrollments (those in session sections of the student's class)
...
export async function getTeacherRelatedRecords(teacherId: string): Promise<RelatedRecordCount> {
  const [classSections, classes, payments] = await Promise.all([
    ...
    prisma.session.count({ where: { teacherId } }),
