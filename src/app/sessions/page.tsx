...
export default function ClassSectionsPage() {
  ...
  const fetchClassSections = async () => {
    try {
      const response = await fetch("/api/session-sections");
      const data = await response.json();
  ...
  const handleCreateClassSection = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Session section created successfully");
        setShowDialog(false);
        setNewClassSection({
          name: "",
          subjectId: "",
        ...
        toast.error("Failed to create session section");
      }
    } catch (error) {
  ...
  const handleDeleteClassSection = async (sectionId: string) => {
    try {
      const response = await fetch(`/api/session-sections/${sectionId}`, {
        method: "DELETE",
      });

      if (response.ok) {
        toast.success("Session section deleted successfully");
        fetchClassSections();
      } else {
        const errorData = await response.json();
        toast.error(errorData.error || "Failed to delete session section");
      }
    } catch (error) {
      console.error("Error deleting session section:", error);
      toast.error("Failed to delete session section");
    }
  };
  ...
  const groupedSections = classSections?.reduce((acc, section) => {
    const levelName = section.subject.class.name;
    if (!acc[levelName]) {
      acc[levelName] = [];
    }
    ...
    <DashboardLayout>
      <div className="p-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold">Session Sections</h1>
            <p className="text-muted-foreground">
              Manage session sections with teachers and schedules
            </p>
          </div>
          <Dialog open={showDialog} onOpenChange={setShowDialog}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Create New Session Section</DialogTitle>
                <DialogDescription>
                  Set up a new session section with teacher and schedule
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleCreateClassSection} className="space-y-4">
                {/* Class Selection */}
                <div>
                  <Label htmlFor="class">Class (Session) *</Label>
                  <Select
                    value={selectedLevel}
                ...
                <div className="flex justify-end gap-2 pt-4">
                  ...
                  <Button type="submit">Create Session Section</Button>
                </div>
              </form>
            </DialogContent>
          </Dialog>
        ...
        {classSections.length === 0 ? (
          <Card>
            <CardHeader>
              <CardTitle>No Session Sections</CardTitle>
              <CardDescription>
                Create your first session section to get started
              </CardDescription>
            </CardHeader>
          </Card>
        ) : (
                            ...
                            <div className="flex items-center">
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <ConfirmationDialog
                                    title="Delete Session Section"
                                    description="Are you sure you want to delete this session section? This action cannot be undone."
