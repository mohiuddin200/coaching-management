...
export function StudentDialog({ student, trigger, onSuccess }: StudentDialogProps) {
  ...
  const form = useForm<StudentFormValues>({
    ...
    defaultValues: {
      ...
      studentPhoneNumber: (student as Student & { studentPhoneNumber?: string })?.studentPhoneNumber || "",
      whatsappNumbers: (student as Student & { whatsappNumbers?: string[] })?.whatsappNumbers || [],
      dateOfBirth: student?.dateOfBirth ? new Date(student.dateOfBirth).toISOString().split('T')[0] : "",
      monthlyFee: (student as Student & { monthlyFee?: number })?.monthlyFee || undefined,
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          monthlyFee: data.monthlyFee !== undefined ? Number(data.monthlyFee) : null,
