 ...
 */
async function testStudentCreationWithAutoEnrollments() {
  console.log('\n=== Test Case 1: Student Creation with Auto-Generated Enrollments ===');
  ...
  try {
    ...
    const allCorrectLevel = student.enrollments.every(e => 
 ...
 */
async function testSoftDeleteWithoutManualEnrollments() {
 ...
 */
async function testSoftDeleteWithManualEnrollments() {
  console.log('\n=== Test Case 3: Soft Delete With Manual Enrollments ===');
  
  try {
    const { level, mathSection, scienceSection } = await createTestLevel();
    ...
    if (!response.ok) {
      ...
      const autoGeneratedRemaining = remainingEnrollments.filter(e => 
        e.classSection.subject.levelId === level.id
      );
      
      const manualRemaining = remainingEnrollments.filter(e => 
